...
(function () {
    ...
    Dobby = {
        ...
        forts: [],
        homeTown: null,
        jobFilter: { filterOnlySilver: false, filterNoSilver: false, filterCenterJobs: false, filterOnlyGold: false, filterJob: "" },
        sortJobTableXp: 0,
    ...
    Dobby.checkIfGold = function (x, y, id) {
        var key = x + "-" + y;
        var jobData = GameMap.JobHandler.Featured[key];
        if (jobData == undefined || jobData[id] == undefined) {
            return false;
        } else {
            return jobData[id].gold;
        }
    }
    ...
    Dobby.getAllUniqueJobs = function () {
        ...
        for (var i = 0; i < Dobby.allJobs.length; i++) {
            ...
            var isGold = Dobby.checkIfGold(currentJob.x, currentJob.y, currentJob.id);
    ...
    Dobby.run = function () {
        Dobby.checkMotivation(0, [], function (result) {
            ...
            } else if ((Character.energy == 0 || Dobby.isHealthBelowLimit()) && Dobby.settings.enableRegeneration == true) {
                Dobby.sleep();
            } else {
    ...
    Dobby.sleep = async function (index) {
        if (Dobby.settings.enableRegeneration && Dobby.selectedSleepPlace != -2) {
            //if sleep place is town
            var equiped = await Dobby.equipSet(Dobby.travelSet);
            await Dobby.sleepInHotel(13);

        }

    }

    Dobby.sleepInHotel = async function (fortId) {
        var equiped = await Dobby.equipSet(Dobby.travelSet);
        const h = "6c10df"; // hotel hash   
        Ajax.remoteCall(
            "task",
            "add",
            {
                "tasks[1][fort_id]": fortId,
                "tasks[1][taskType]": "fortsleep"
            },
            async function (response)  {
                 var sleepEquiped = await Dobby.equipSet(Dobby.regenerationSet);
    ...
    Dobby.createJobsTab = function () {
        ...
        var checkboxOnlyGold = new west.gui.Checkbox();
        checkboxOnlyGold.setLabel("Golds");
        checkboxOnlyGold.setSelected(Dobby.jobFilter.filterOnlyGold);
        checkboxOnlyGold.setCallback(function () {
            if (this.isSelected()) {
                Dobby.jobFilter.filterOnlyGold = true;
            } else {
                Dobby.jobFilter.filterOnlyGold = false;
            }
        });
        ...
        $("#job_only_gold", html).append(checkboxOnlyGold.getMainDiv());
